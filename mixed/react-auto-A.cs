...
public static class ReactAutoA_Tests {
  public static void Run() {
    ...
    if (testAuto.ARectOutput.ToString() != aRect) {
      Console.WriteLine("ERROR: ARectOutput = {0}, aRect = {1}", testAuto.ARectOutput, aRect);
      return;
    }

    if (testAuto.ADateRecOutput.ToString() != aDateRec) {
      Console.WriteLine("ERROR: aDateRecOutput = {0}, aDateRec = {1}", testAuto.ADateRecOutput, aDateRec);
      return;
    }
    ...
    if (testAuto.APointOutput.ToString() != aPoint) {
      Console.WriteLine("ERROR: APointOutput = {0}, APoint = {1}", testAuto.APointOutput, aPoint);
      return;
    }

    if (testAuto.AnAnyPointOutput.ToString() != anAnyPoint) {
    ...
    if (!EqVS(testAuto.APointSeqOutput, aPointSeq)) {
      Console.WriteLine("ERROR: APointSeqOutput = {0}, APointSeq = {1}", testAuto.APointSeqOutput, aPointSeq);
      return;
    }
    ...
    if (testAuto.AnIntToSymbMapOutput.ToString() != anIntToSymbMap) {
      Console.WriteLine("ERROR: AnIntToSymbMapOutput = {0}, anIntToSymbMap = {1}", testAuto.AnIntToSymbMapOutput, anIntToSymbMap);
      return;
    }
  ...
  static bool EqVS(Value[] vs, string[] ss) {
    if (vs.Length != ss.Length)
      return false;
    for (int i=0 ; i < vs.Length ; i++)
      if (vs[i].ToString() != ss[i])
        return false;
    return true;
  }
}
